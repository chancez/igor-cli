...
def make_api_request(method, config, endpoint, **kwargs):
    """Makes the Igor API request and handles 400, 401, 403 and connection errors"""
    try:
        ...
        if response.status_code != requests.codes.ok and \
           response.status_code != requests.codes.created:
            if response.status_code == requests.codes.unauthorized:
                print 'It appears you haven\'t logged in or your token has expired.',
                print 'See \'igor auth\'.'
            elif response.status_code == requests.codes.forbidden:
                print 'It appears you don\'t have permission for this resource.',
                print 'See \'igor permissions\'.'
            else:
                print 'Igor says: "' + response.json()['message'] + '"',
